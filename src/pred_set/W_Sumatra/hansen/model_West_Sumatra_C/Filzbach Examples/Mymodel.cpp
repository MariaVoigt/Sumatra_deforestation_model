...
//W_Sumatra
#define GRIDNX 1952
#define GRIDNY 2675 // dimensions of grid we are using
#define XLLCORNER -2826358.004000000190
#define YLLCORNER  1246689.379999999888// coerner dimensions
...
mybeta_8, mybeta_9, mybeta_10, gampeat[1], gammine[2], gamlu[5], myrho;
int beta0_global = 1;
...
double get_pdefor(int xx, int yy, int datamod)
{
	...
	double pdefor, k, mygamma_peat, mygamma_mine, mygamma_lu, mygamma_b0;
...
void applyModel(int plookup)
{
	...
	for (j = 0; j < iterations; j++)
	{
		...
		//	sprintf(fname, "E:/Sumatra_model_August22/hansen/W_Sumatra/model_W_Sumatra_C/predprob_yr%d_i%d.asc", n, j);
		...
		//	sprintf(fname, "E:/Sumatra_model_August22/hansen/W_Sumatra/model_W_Sumatra_C/predfor_i%d_%dyr.asc", j, n);
		//	writeAsciiGrid(fname, modelForest, GRIDNX, GRIDNY, XLLCORNER, YLLCORNER, CELLSIZE, -9999.0);
			...
			sprintf(fname, "E:/Sumatra_model_August22/hansen/W_Sumatra/model_W_Sumatra_C/preddef_i%d_%dyr.asc", j, n);
...
void read_data()
{
	...
	//EDIT
	readAsciiGrid("./workspace/forest_2017_21_180m_repro_res_hansen_W_Sumatra.ascii", forest);		//
	readAsciiGrid("./workspace/deforestation_2017_21_180m_repro_res_hansen_W_Sumatra.ascii", def);		//
	readAsciiGrid("./workspace/slope_180m_repro_res_hansen_W_Sumatra.ascii", slope);		//
	readAsciiGrid("./workspace/fire_yearly_average_180m_repro_res_hansen_W_Sumatra.ascii", fire);		//
	readAsciiGrid("./workspace/IDN_TTCSM_hrs_180m_repro_res_hansen_W_Sumatra.ascii", access_hrs);		//
	readAsciiGrid("./workspace/road_distance_180m_repro_res_hansen_W_Sumatra.ascii", roads);		//
	readAsciiGrid("./workspace/river_distance_180m_repro_res_hansen_W_Sumatra.ascii", rivers);		//
	readAsciiGrid("./workspace/pressurelog10_sigma1_180m_repro_res_hansen_W_Sumatra.ascii", pp_1);		//
	readAsciiGrid("./workspace/subsistence_LH_distance_180m_repro_res_hansen_W_Sumatra.ascii", subsistenceLH);		//
	readAsciiGrid("./workspace/non_agri_LH_distance_180m_repro_res_hansen_W_Sumatra.ascii", non_agriLH);		//
	readAsciiGrid("./workspace/soc_econMPI_180m_repro_res_hansen_W_Sumatra.ascii", mpi);		//

	readAsciiGrid("./workspace/peat_180m_repro_res_hansen_W_Sumatra.ascii", peat);		//peat
	readAsciiGrid("./workspace/mining_180m_repro_res_hansen_W_Sumatra.ascii", mining);		//mining
	readAsciiGrid("./workspace/lu_new_class_180m_repro_res_hansen_W_Sumatra.ascii", landuse);		//landuse
	return;
...
void setup_model()
{
	...
	parameter_create("beta_3", 10.0, 30.0, 0.0, 0, 0, 1);		//fire
	...
	parameter_create("beta_9", -1.0, 1.0, 0.0, 0, 0, 1);		//distance to non_agri
	parameter_create("beta_10", -6.0, 4.0, 0.0, 0, 0, 1);		//mpi
	...
	for (turn = 0; turn <= npara; turn++)
	{
		...
		for (tp = -1; tp < npara; tp++)
		{
			...
			else
			{
				...
				beta0_global = 1;
				setup_model();
				sprintf(aname, "´W_Sumatra_1_%d", rr);
