...
void applyModel(int plookup)
{
	...
	for (j = 0; j < iterations; j++)
	{
		...
		for (n = 0; n < numsteps; n++)
		{
			...
			sprintf(fname, "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_D/preddef_i%d_%dyr.asc", j, n);
...
void read_data()
{
	...
	readAsciiGrid("./workspace/road_distance_180m_repro_res_tmf_Jambi.ascii", roads);		//
	...
	readAsciiGrid("./workspace/plantation_LH_distance_180m_repro_res_tmf_Jambi.ascii", plantationLH);		//
...
int main()
{
	...
	for (turn = 0; turn <= npara; turn++)
	{
		...
		for (tp = -1; tp < npara; tp++)
		{
			...
			else
			{
				...
				table_output("model_likelihood", "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_D/TempLhoodDdef_1.txt");
	...
	//output likelihood table 
	table_output("model_likelihood", "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_D/Models_Stepwise_1.txt");
