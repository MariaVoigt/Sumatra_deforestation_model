...
mybeta_8, mybeta_9, gampeat[1], gampiaps[2], gammine[2], gamlu[5],  myrho;
int beta0_global = 1;

// Maria - change here for the total number of parameters
int tfree[13]; // for stepwise regression, flag for whether parameters are currently free (1), or fixed at 0.0 (0) 
...
double get_pdefor(int xx, int yy, int datamod)
{
	...
	k = mybeta_0 + mybeta_1 * get_neib(xx, yy, datamod, myrho) + 
		...
		mybeta_5 * rivers[xx][yy] +
		mybeta_6 * pp_1[xx][yy] +
		mybeta_7 * subsistenceLH[xx][yy] + 
		mybeta_8 * non_agriLH[xx][yy] + 
		mybeta_9 * mpi[xx][yy] +
...
void applyModel(int plookup)
{
	...
	if (first == 1)
	{
		...
		table_addcolumn("parameters", "gamlu_3");
		table_addcolumn("parameters", "gamlu_4");
	}
	...
	for (j = 0; j < iterations; j++)
	{
		...
		for (n = 0; n < numsteps; n++)
		{
			...
			//sprintf(fname, "E:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/predprob_yr%d_i%d.asc", n, j);
			...
			//sprintf(fname, "E:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/predfor_i%d_%dyr.asc", j, n);
			...
			sprintf(fname, "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/preddef_i%d_%dyr.asc", j, n);
		...
		table_writevalue("parameters", "gamlu_3", j, cv("gamma_lu", 3));
		table_writevalue("parameters", "gamlu_4", j, cv("gamma_lu", 4));

		//Export forestdata table
		sprintf(fname, "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/Model_35yrs_temp.txt");
	  ...
	  //EDIT
	  //Export forestdata table
	sprintf(fname, "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/Model_35yrs.txt");
	table_output("forestdata", fname);

	//Export parameters table 
	table_output("parameters", "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/Parameters_Values.txt");
...
void read_data()
{
	...
	//EDIT
	readAsciiGrid("./workspace/forest_2017_21_180m_repro_res_tmf_Jambi.ascii", forest);		//
	readAsciiGrid("./workspace/deforestation_2017_21_180m_repro_res_tmf_Jambi.ascii", def);		//
	readAsciiGrid("./workspace/slope_180m_repro_res_tmf_Jambi.ascii", slope);		//
	readAsciiGrid("./workspace/fire_yearly_average_180m_repro_res_tmf_Jambi.ascii", fire);		//
	readAsciiGrid("./workspace/IDN_TTCSM_hrs_180m_repro_res_tmf_Jambi.ascii", access_hrs);		//
	readAsciiGrid("./workspace/river_distance_180m_repro_res_tmf_Jambi.ascii", rivers);		//
	readAsciiGrid("./workspace/pressurelog10_sigma1_180m_repro_res_tmf_Jambi.ascii", pp_1);		//
	readAsciiGrid("./workspace/subsistence_LH_distance_180m_repro_res_tmf_Jambi.ascii", subsistenceLH);		//
	readAsciiGrid("./workspace/non_agri_LH_distance_180m_repro_res_tmf_Jambi.ascii", non_agriLH);		///
	readAsciiGrid("./workspace/soc_econMPI_180m_repro_res_tmf_Jambi.ascii", mpi);		//

	readAsciiGrid("./workspace/peat_180m_repro_res_tmf_Jambi.ascii", peat);		//peat
	readAsciiGrid("./workspace/piaps_180m_repro_res_tmf_Jambi.ascii", piaps);		//
	readAsciiGrid("./workspace/mining_180m_repro_res_tmf_Jambi.ascii", mining);		//mining
	readAsciiGrid("./workspace/lu_new_class_180m_repro_res_tmf_Jambi.ascii", landuse);		//landuse
	return;
...
void setup_model()
{
	...
	parameter_create("beta_5", -1.0, 1.0, 0.0, 0, 0, 1);		//rivers
	parameter_create("beta_6", -2.0, 2.0, 0.0, 0, 0, 1);		//population pressure sigma 1
	parameter_create("beta_7", -1.0, 1.0, 0.0, 0, 0, 1);		//distance to subsistence
	parameter_create("beta_8", -1.0, 1.0, 0.0, 0, 0, 1);		//distance to non_agri
	parameter_create("beta_9", -2.0, 2.0, 0.0, 0, 0, 1);		//mpi

	parameter_create_vector("gamma_peat", -5.0, 2.0, 0.0, 0, 0, 1, 1);// // last digit is 1+ the last element in get_params, to allow a parameter value for each class (excluding zero!)
...
int main()
{
	...
	int pfree[13]; // this is 1 if the parameter is set to be ALWAYS free
	int npara = 13; // number of params we are considering as fixed or not
	...
	table_addcolumn("model_likelihood", "gamma_3lu");
	table_addcolumn("model_likelihood", "gamma_4lu");
	...
	for (turn = 0; turn <= npara; turn++)
	{
		...
		for (tp = -1; tp < npara; tp++)
		{
			...
			else
			{
				...
				table_writevalue("model_likelihood", "gamma_peat", rr, tfree[12]);
				table_writevalue("model_likelihood", "gamma_piaps", rr, tfree[13]);
				...
				table_writevalue("model_likelihood", "gamma_3lu", rr, cv("gamma_lu", 3));
				table_writevalue("model_likelihood", "gamma_4lu", rr, cv("gamma_lu", 4));


				//output likelihood table 
				table_output("model_likelihood", "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/TempLhoodDdef_1.txt");
	...
	//output likelihood table 
	table_output("model_likelihood", "M:/Sumatra_model_August22/tmf/Jambi/model_Jambi_F/Models_Stepwise_1.txt");
