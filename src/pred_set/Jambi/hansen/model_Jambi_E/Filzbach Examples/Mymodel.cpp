...
mybeta_8, mybeta_9, mybeta_10,  gampeat[1], gampiaps[2], gammine[2], gamlu[5], myrho;
int beta0_global = 1;
...
void read_data()
{
	...
	readAsciiGrid("./workspace/road_distance_180m_repro_res_hansen_Jambi.ascii", roads);		//
...
void setup_model()
{
	...
	parameter_create("beta_10", -2.0, 2.0, 0.0, 0, 0, 1);		//mpi
	...
	for (turn = 0; turn <= npara; turn++)
	{
		...
		for (tp = -1; tp < npara; tp++)
		{
			...
			else
			{
				...
				table_writevalue("model_likelihood", "gamma_piaps", rr, tfree[11]);
