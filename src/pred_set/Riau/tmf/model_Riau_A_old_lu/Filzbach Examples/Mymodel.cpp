...
void applyModel(int plookup)
{
	...
	for (j = 0; j < iterations; j++)
	{
		...
		for (n = 0; n < numsteps; n++)
		{
			...
			sprintf(fname, "E:/Sumatra_model_August22/tmf/Riau/model_Riau_A/preddef_i%d_%dyr.asc", j, n);
...
void read_data()
{
	...
	//EDIT
	readAsciiGrid("./workspace/forest_2017_21_180m_repro_res_tmf_Riau.ascii", forest);		//
	readAsciiGrid("./workspace/deforestation_2017_21_180m_repro_res_tmf_Riau.ascii", def);		//
	readAsciiGrid("./workspace/slope_180m_repro_res_tmf_Riau.ascii", slope);		//
	readAsciiGrid("./workspace/fire_yearly_average_180m_repro_res_tmf_Riau.ascii", fire);		//
	readAsciiGrid("./workspace/IDN_TTCSM_hrs_180m_repro_res_tmf_Riau.ascii", access_hrs);		//
	readAsciiGrid("./workspace/road_distance_180m_repro_res_tmf_Riau.ascii", roads);		//
	readAsciiGrid("./workspace/river_distance_180m_repro_res_tmf_Riau.ascii", rivers);		//
	readAsciiGrid("./workspace/pressurelog10_sigma1_180m_repro_res_tmf_Riau.ascii", pp_1);		//
	readAsciiGrid("./workspace/subsistence_LH_distance_180m_repro_res_tmf_Riau.ascii", subsistenceLH);		//
	readAsciiGrid("./workspace/plantation_LH_distance_180m_repro_res_tmf_Riau.ascii", plantationLH);		//
	readAsciiGrid("./workspace/non_agri_LH_distance_180m_repro_res_tmf_Riau.ascii", non_agriLH);		//
	readAsciiGrid("./workspace/transmigrant_distance_180m_repro_res_tmf_Riau.ascii", transmigrant);		//

	readAsciiGrid("./workspace/peat_180m_repro_res_tmf_Riau.ascii", peat);		//peat
	readAsciiGrid("./workspace/piaps_180m_repro_res_tmf_Riau.ascii", piaps);		//
	readAsciiGrid("./workspace/plantations_180m_repro_res_tmf_Riau.ascii", plantations);		//
	readAsciiGrid("./workspace/mining_180m_repro_res_tmf_Riau.ascii", mining);		//mining
	readAsciiGrid("./workspace/lu_new_class_180m_repro_res_tmf_Riau.ascii", landuse);		//landuse
	return;
...
void setup_model()
{
	...
	if (beta0_global == 1)
		...
		parameter_create_vector("gamma_b0", -6.0, 6.0, 0.0, 0, 0, 1, 9);	//vector to store different b0 for each state (not used...)
																			...
																			// continuous variables
	parameter_create("beta_1", -2.0, 5.0, 0.0, 0, 0, 1);		//proportion of deforested neighbours (updated every year, dynamic variable)
	...
	parameter_create_vector("gamma_peat", -2.0, 2.0, 0.0, 0, 0, 1, 1);// // last digit is equal the highest class and 1+ the last element in get_params, to allow a parameter value for each class (excluding zero!)

	parameter_create_vector("gamma_piaps", -2.0, 2.0, 0.0, 0, 0, 1, 2);// // last digit is equal the highest class and last digit is 1+ the last element in get_params, to allow a parameter value for each class (excluding zero!)

	parameter_create_vector("gamma_plant", -2.0, 2.0, 0.0, 0, 0, 1, 2);// //last digit is equal the highest class and  last digit is 1+ the last element in get_params, to allow a parameter value for each class (excluding zero!)

	parameter_create_vector("gamma_mine", -2.0, 2.0, 0.0, 0, 0, 1, 2);// // last digit is equal the highest class and last digit is 1+ the last element in get_params, to allow a parameter value for each class (excluding zero!)

	parameter_create_vector("gamma_lu", -2.0, 6.0, 0.0, 0, 0, 1, 5);// EDIT
