...
float landuse[GRIDNX][GRIDNY];        //landuse, 4 levels,0 - 3 
									 // 
...
int yval[GRIDNX*GRIDNY]; //vector to store sample columns ID
						 ...
						 //
double mybeta_0, mybeta_1, mybeta_2, gamlu[3],  myrho;
int beta0_global = 1;

// Maria - change here for the total number of parameters
int tfree[4]; // for stepwise regression, flag for whether parameters are currently free (1), or fixed at 0.0 (0) 
...
double get_pdefor(int xx, int yy, int datamod)
{
	...
	double pdefor, k, mygamma_lu, mygamma_b0;
	...
	//model
	k = mybeta_0 + mybeta_1*get_neib(xx, yy, datamod, myrho) + mybeta_2 * slope[xx][yy] + 
...
void applyModel(int plookup)
{
	...
	for (j = 0; j<iterations; j++)
	{
		...
		for (n = 0; n<numsteps; n++)
		{
			...
			sprintf(fname, "E:/Sumatra_model_August22/tmf/N_Sumatra/model_N_Sumatra_test/predprob_yr%d_i%d.asc", n, j);
			...
			sprintf(fname, "E:/Sumatra_model_August22/tmf/N_Sumatra/model_N_Sumatra_test/predfor_i%d_%dyr.asc", j, n);
			...
			sprintf(fname, "E:/Sumatra_model_August22/tmf/N_Sumatra/model_N_Sumatra_test/preddef_i%d_%dyr.asc", j, n);
...
int main()
{
	...
	int pfree[4]; // this is 1 if the parameter is set to be ALWAYS free
	int npara = 4; // number of params we are considering as fixed or not
	...
	for (turn = 0; turn <= npara; turn++)
	{
		...
		for (tp = -1; tp<npara; tp++)
		{
			...
			else
			{
				...
				table_writevalue("model_likelihood", "gamma_lu", rr, tfree[11]);
