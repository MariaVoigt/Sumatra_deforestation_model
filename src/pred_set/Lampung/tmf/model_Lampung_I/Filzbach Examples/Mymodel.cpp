...
float ind_plantations[GRIDNX][GRIDNY];	    //distance to indscale plantations
...
void applyModel(int plookup)
{
	...
	for (j = 0; j < iterations; j++)
	{
		...
		for (n = 0; n < numsteps; n++)
		{
			...
			//sprintf(fname, "M:/Sumatra_model_August22/tmf/Lampung/model_Lampung_I/predprob_yr%d_i%d.asc", n, j);
			...
			//sprintf(fname, "M:/Sumatra_model_August22/tmf/Lampung/model_Lampung_I/predfor_i%d_%dyr.asc", j, n);
			...
			sprintf(fname, "M:/Sumatra_model_August22/tmf/Lampung/model_Lampung_I/preddef_i%d_%dyr.asc", j, n);
		...
		sprintf(fname, "M:/Sumatra_model_August22/tmf/Lampung/model_Lampung_I/Model_35yrs_temp.txt");
	  ...
	  //EDIT
	  //Export forestdata table
	sprintf(fname, "M:/Sumatra_model_August22/tmf/Lampung/model_Lampung_I/Model_35yrs.txt");
	table_output("forestdata", fname);

	//Export parameters table 
	table_output("parameters", "M:/Sumatra_model_August22/tmf/Lampung/model_Lampung_I/Parameters_Values.txt");
...
void read_data()
{
	...
	//EDIT
	readAsciiGrid("./workspace/forest_2017_21_180m_repro_res_tmf_Lampung.ascii", forest);		//
	readAsciiGrid("./workspace/deforestation_2017_21_180m_repro_res_tmf_Lampung.ascii", def);		//
	readAsciiGrid("./workspace/slope_180m_repro_res_tmf_Lampung.ascii", slope);		//
	readAsciiGrid("./workspace/fire_yearly_average_180m_repro_res_tmf_Lampung.ascii", fire);		//
	readAsciiGrid("./workspace/IDN_TTCSM_hrs_180m_repro_res_tmf_Lampung.ascii", access_hrs);		//
	readAsciiGrid("./workspace/road_distance_180m_repro_res_tmf_Lampung.ascii", roads);		//
	readAsciiGrid("./workspace/river_distance_180m_repro_res_tmf_Lampung.ascii", rivers);		//
	readAsciiGrid("./workspace/pressurelog10_sigma1_180m_repro_res_tmf_Lampung.ascii", pp_1);		//
	readAsciiGrid("./workspace/subsistence_LH_distance_180m_repro_res_tmf_Lampung.ascii", subsistenceLH);		//
	readAsciiGrid("./workspace/plantation_LH_distance_180m_repro_res_tmf_Lampung.ascii", plantationLH);		//
	readAsciiGrid("./workspace/non_agri_LH_distance_180m_repro_res_tmf_Lampung.ascii", non_agriLH);		//
	readAsciiGrid("./workspace/transmigrant_distance_180m_repro_res_tmf_Lampung.ascii", transmigrant);		//
	readAsciiGrid("./workspace/ind_plantations_distance_180m_repro_res_tmf_Lampung.ascii", ind_plantations);		//
	readAsciiGrid("./workspace/soc_econMPI_180m_repro_res_tmf_Lampung.ascii", mpi);		//

	readAsciiGrid("./workspace/peat_180m_repro_res_tmf_Lampung.ascii", peat);		//peat
	readAsciiGrid("./workspace/piaps_180m_repro_res_tmf_Lampung.ascii", piaps);		//
	readAsciiGrid("./workspace/mining_180m_repro_res_tmf_Lampung.ascii", mining);		//mining
	readAsciiGrid("./workspace/lu_new_class_180m_repro_res_tmf_Lampung.ascii", landuse);		//landuse
	return;
...
void setup_model()
{
	...
	parameter_create("beta_12", -2.0, 2.0, 0.0, 0, 0, 1);		//distance ind scale
	parameter_create("beta_13", -10.0, 2.0, 0.0, 0, 0, 1);		//mpi

	parameter_create_vector("gamma_peat", -6.0, 2.0, 0.0, 0, 0, 1, 1);// // last digit is 1+ the last element in get_params, to allow a parameter value for each class (excluding zero!)
...
int main()
{
	...
	tfree[11] = 1; //dist ind scale plant
	tfree[12] = 1; //mpi
