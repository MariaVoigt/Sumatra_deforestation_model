...
list_units_hansen = rbind(c(1, "Aceh", "Aceh", "A"),
                          ...
                          c(3, "W_Sumatra", "West Sumatra", "C"),
                          c(4, "Riau", "Riau", "A"),
                          c(5, "Jambi", "Jambi", "E"),
                          c(6, "Bengkulu", "Bengkulu", "G"),
                          c(7, "S_Sumatra", "South Sumatra", "A"),
                          c(8, "Lampung", "Lampung",  "H"))
...
cbPalette_custom <- hex_codes2[c(1,3,5,9,
                                 26,33,35,
                                38, 47)]
...
write.csv(table_S3,  file.path(out_path, paste0("table_coefficient_values_units_",forest_layer, ".csv")), row.names = F)
...
pdf(file.path(out_path, paste0("Parameter_effect_size_all_",forest_layer,".pdf")), width =0, height = 0, paper = "a4r")
print(p1)
dev.off()
...
pdf(file.path(out_path, paste0("Parameter_effect_size_important_",forest_layer,".pdf")), width =0, height = 0, paper = "a4r")
print(plot2)
dev.off()
...
# 
# 
# pdf(file.path(out_path, "Parameter_effect_size_no_jitter.pdf"), width =0, height = 0, paper = "a4r")
# ggplot(data = parameters_m_s_units_filter ) + # 
#   geom_hline(yintercept = 0, color = "black", size = 1, linetype = "dashed")+
#   geom_boxplot(aes(y = value, x = pred_names_char ), color = "black", fill = "gray89", width = 0.8, outlier.shape=NA)+
#  # geom_jitter(aes( y = value, x = pred_names_char , color = unit_long, shape = unit_long), size=3, width = 0.3, alpha = 0.8)+
# #  scale_shape_manual(values = shape_custom)+
#   coord_flip()+
#   # labs(fill = "test") +
#   theme_bw() +
...
# dev.off()
# # 
# pdf(file.path(out_path, "Parameter_effect_size_only_jitter.pdf"), width =0, height = 0, paper = "a4r")
# ggplot(data = parameters_m_s_units_filter ) + # 
#   geom_hline(yintercept = 0, color = "black", size = 1, linetype = "dashed")+
#  # geom_boxplot(aes(y = value, x = pred_names_char ), color = "black", fill = "gray89", width = 0.8, outlier.shape=NA)+
#    geom_jitter(aes( y = value, x = pred_names_char , color = unit_long, shape = unit_long), size=3.5, width = 0.3, alpha = 0.8)+
#     scale_shape_manual(values = shape_custom)+
#   coord_flip()+
#   # labs(fill = "test") +
#   theme_bw() +
...
# 
# pdf(file.path(out_path, "Parameter_effect_size_region_based.pdf"), width =0, height = 0, paper = "a4r")
# ggplot(data = parameters_m_s_units_filter ) + # 
#   geom_hline(yintercept = 0, color = "black", size = 1, linetype = "dashed")+
#   # geom_boxplot(aes(y = value, x = pred_names_char ), color = "black", fill = "gray89", width = 0.8, outlier.shape=NA)+
#   geom_jitter(aes( y = value, x = unit_long, color = pred_names_char), size=3.5, width = 0.3, alpha = 0.8)+ #, shape =  pred_names_char
#  # scale_shape_manual(values = shape_custom)+
#   coord_flip()+
#   # labs(fill = "test") +
#   theme_bw() +
